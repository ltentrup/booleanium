...
* **How much does gate *detection* on CNF actually recover? —
  ...
  bottleneck.
* **Is the determinacy check itself the bottleneck? No — and trying to
  make it one showed the survey's predictor is a marker, not a lever.**
  The fraction determinized up front is the strongest predictor of
  solvability above, so the obvious follow-up is whether the *check* is
  ...
  **on their own**, admit an assignment under which none of them fires,
  ...
  Closing that is straightforward: ask the same question against the
  determinized formula the conflict-check solver already holds. Each
  implication clause `C` contributes `C \ {v}` ("this one does not
  fire"), and unsatisfiability of the conjunction means `v` is forced
  under every universal assignment and every choice of the still-open
  variables. Implemented as `is_determined_globally` /
  `Options::deep_determinacy`, root level only, fuzz-verified against
  the brute-force oracle in the differential option sweep.

  It finds nothing on structured circuits, for a structural reason:
  **the inputs of a gate are independent by construction**, so the gap
  the local check finds is genuinely reachable and the extra context
  adds no constraint. On `parity`, `mux-tree` and `bv-add-inverse` the
  strengthened check is never even *reached* at the root — every local
  check already answers Deterministic or Constant. On `choice-16` and
  `bv-ult-choice-8` it runs 33–64 times and determinizes **zero**.

  On random circuits, where gate inputs are drawn independently and
  reconvergent fanout correlates the premises, it does fire — and
  **costs more than it returns**. Over 36 instances
  (`random-6-10-40-01…24`, `random-8-14-60-01…12`), two runs:

  | | baseline | deep | |
  |---|---|---|---|
  | total time | 46.0 / 43.9 ms | 62.2 / 60.7 ms | **1.35–1.38x slower** |
  | total decisions | 1 946 | 2 099 | 1.08x more |
  | instances >10% faster | | 8 of 36 | |
  | instances >10% slower | | **17 of 36** | |
  | determinization moved | | **3 of 36** | |

  The decision counts are deterministic and identical across runs, so
  the regression is a property of the search, not timing noise.

  **An earlier version of this entry claimed the opposite** — a ~1.6x
  speedup with 37–56% fewer decisions — on the strength of three random
  seeds, and called them "clausal instances", which they are not: they
  are random *circuits*. Both were wrong. Widening to 36 instances
  reverses the sign.

  The interesting part is *why* it does not pay, because it says
  something about the survey. Determinization moved on three instances,
  and on one of them the extra determinization made the solve
  distinctly **worse** (`random-8-14-60-10`: 7 → 8 determinized up
  front, but 110 → 193 decisions and 2.08 → 5.45 ms). So intervening on
  the predictor does not produce the outcome the correlation suggests.
  The determinized fraction is a **marker of instances that are easy,
  not a lever that makes them easy** — which is the causal caveat the
  survey flagged ("instance size confounds"), now with direct evidence
  rather than a hedge.

  So low determinization on hard QBFEVAL instances is not a weak-check
  problem, and forcing the number up is not the fix either. What is
  missing on those instances is the definitions themselves, destroyed
  by one-sided encoding or never present, which argues for
